...
#        self.layer0 = nn.Sequential(torch.nn.Linear(self.input_dim,512))
       ...
       #  self.layer1.add_module('lstm', nn.LSTM(self.input_dim, self.hidden_dim, num_layers=2, batch_first=True, bidirectional=False))
        self.layer1 = nn.Sequential(nn.LSTM(self.input_dim, self.hidden_dim/2, num_layers=1, batch_first=True, bidirectional=True))
		#        self.layer1.add_module('lstm', nn.LSTM(self.input_dim, self.hidden_dim/2, num_layers=2, batch_first=True, bidirectional=True))
